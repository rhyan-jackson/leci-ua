...
int ListSearch(List* l, const void* p) {
  compareFunc = l->compare;
  // TODO: Optimize

  ListNode* currentNodeAux = l->head;
  int currentNodePosAux = 0;

  while(currentNodeAux != NULL) {
    int compResult = compareFun(p, currentNodeAux->item);

    if (compResult < 0) break;
    if (compResult == 0) {
      l->current = currentNodeAux;
      l->currentPos = currentNodePosAux;
      return 0;
    }
