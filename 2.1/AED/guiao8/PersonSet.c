...
PersonSet *PersonSetCreate() {
  ...
  PersonSet* ppersonset = (PersonSet*) malloc(sizeof(PersonSet));
  Person **array = (Person**) malloc(4 * sizeof(Person*));

  if (ppersonset == NULL || array == NULL) {
    perror("PersonSetCreate");
    free(ppersonset);
    exit(2);
  }

  ppersonset->capacity = INITIAL_CAPACITY;
  ppersonset->size = 0;
  ppersonset->array = array;
